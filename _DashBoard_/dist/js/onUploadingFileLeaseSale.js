...
// on uploading file 
function onSubmit(){
    alert('je suis le meilleur de tout le monde')
}
function addingPrd(evt){
    const btn = document.getElementById('onSending-off');
    if ($('#addprd')[0].checkValidity()) {
        ...
        // alert('valid');
        const formdata = new FormData(document.getElementById('addprd'));
        var xhr = new XMLHttpRequest();
        xhr.open('POST','dist/js/_phpSubmition.hnd.php',true);
        xhr.send(formdata);
        xhr.onreadystatechange = function(){
          if(xhr.status === 200 && xhr.readyState === 4)  {
            if(xhr.response.indexOf(xhr.status) != -1){
                btn.removeChild(span);
                alert('lesmilleservices\rProduit ajouté avec success\rby davidmened@gmail.com');
            }else{
            btn.removeChild(span);
            console.log(xhr.response);
            alert('lesmilleservices\rUne erreur est survenue peut etre que les fichiers sont trops grands ou la connexion n\'est pas bonne\rby davidmened@gmail.com'); 
          }
        }
    }
}
