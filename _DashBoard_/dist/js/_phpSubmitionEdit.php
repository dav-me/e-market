...
// var_dump($_FILES);
// echo('tout le monde sait que david est meilleur des tous');
$error_message = [];
$arrayFile = [];
if(isset($_POST['idmessage']) && isset($_POST['response'])){
    if(_on_responding_customer(trim($_POST['idmessage']),trim($_POST['response']),$bdd)){
        echo(200);
    }else{
        echo(500);
    }
    // echo('david le grand maitre');
}
if(isset($_GET['_delmessage'])){
    $item = @htmlspecialchars(trim($_GET['_delmessage']));
    if(_deleteMessage($item,$bdd)){
        echo(200);
    }else echo(500);
}
